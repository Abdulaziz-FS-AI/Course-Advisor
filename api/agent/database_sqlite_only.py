    ...
    def __init__(self, db_path: Path = DB_PATH):
        ...
        if is_vercel:
            ...
            if tmp_path.exists():
                ...
                print(f"⚠️ WARNING: Database is in /tmp (ephemeral storage)!")
                print(f"⚠️ Data will be LOST on function restart!")
                print(f"⚠️ Use Vercel Postgres or external DB for persistence!")
            else:
...
"""
    ...
    def create_chat_session(self, title: str, session_id: str, device_id: str) -> Dict[str, Any]:
        ...
        print(f"[DB] Creating session - id: {session_id}, device: {device_id}, title: {title}")
    ...
    def add_message(self, session_id: str, role: str, content: str) -> Dict[str, Any]:
        ...
        try:
            print(f"[DB] Adding message - session: {session_id}, role: {role}, db_path: {self.db_path}")
            with self.get_connection() as conn:
                cursor = conn.cursor()
                # Insert message
                cursor.execute("""
                ...
                print(f"[DB] Message inserted with ID: {message_id}")
                ...
                conn.commit()
                print(f"[DB] Message saved successfully. DB file size: {self.db_path.stat().st_size} bytes")
    ...
    def create_feedback(self, user_id: int, session_id: str, message_id: int, rating: str, comment: str = None) -> Dict[str, Any]:
        ...
        print(f"[DB] Creating feedback - session: {session_id}, message: {message_id}, rating: {rating}")
        ...
        try:
            with self.get_connection() as conn:
                ...
                print(f"[DB] Feedback created with ID: {feedback_id}")
                # Return the created feedback
                cursor.execute("SELECT * FROM feedback WHERE id = ?", (feedback_id,))
                row = cursor.fetchone()
                columns = [desc[0] for desc in cursor.description]
                result = dict(zip(columns, row))
                print(f"[DB] Feedback saved successfully. DB file size: {self.db_path.stat().st_size} bytes")
                return result
        except sqlite3.Error as e:
            print(f"[DB ERROR] Failed to create feedback: {e}")
